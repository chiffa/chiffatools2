...
def submatrix(matrix, indexes, show=False):
    ...
    if re_matrix.shape[0] > 1:
        return np.sum(np.triu(re_matrix.toarray(), 1)) / \
               (re_matrix.shape[0] * (re_matrix.shape[0] - 1) / 2)
    else:
        return 0
...
def show_eigenvals_and_eigenvects(
        eigvals, eigvects, biggest_limit, annotation, idx_chars=None):
    print annotation
    print '########################'
    if not idx_chars:
    ...
    for i, eigval in enumerate(reversed(eigvals.tolist())):
        ...
        for index, value in reversed(
                zip(eigbiggest[:, i], absolute[eigbiggest[:, i], i])):
            if int(value ** 2 * 1000) > 0:
                mset.add(index)
                renderstring.append('\t' +
                                    str(index) +
                                    '\t' +
                                    str('{0:.4f}'.format(value ** 2)) +
                                    '\t' +
                                    str(idx_chars[index]))
        renderstring.append('<==================>')
...
def analyze_eigvects(non_normalized_Laplacian, num_first_eigvals_to_analyse,
                     index_chars, permutations_limiter=10000000, fudge=10e-10):
    ...
    true_eigenvals, true_eigenvects = eigsh(
        normalized_Laplacian, num_first_eigvals_to_analyse)
    print time() - t
    t = time()
    ...
    diagterms = [-item for sublist in fullmat.sum(axis=0).tolist()
                 for item in sublist]
    ...
    rand_eigenvals, rand_eigenvects = eigsh(
        normalized_rand, num_first_eigvals_to_analyse)
    print time() - t
    t = time()
    show_eigenvals_and_eigenvects(
        ...
        index_chars)
    show_eigenvals_and_eigenvects(
        rand_eigenvals, rand_eigenvects, 20, 'random')
...
def hierchical_clustering(distance_matrix, names, noshow=False):
    """
    A set of tools to compute hierarchical clustering over a matrix with names. Taken from scipy examples page

    :param distance_matrix: matrix of distances used for clustering
    :param names:
    :param noshow:
    :return: index that would perform the matrix clustering
    """
...
def show_matrix_with_names(matrix, vert_names, horiz_names,
                           colormap='b_jet', overlay=None, force_range=False):
    """
    A shortcut for plotting a matrix with line and column names. Deprecated

    :param matrix: Matrix to plot
    :param vert_names: line names
    :param horiz_names: column names
    :param colormap: names of desired colormap
    :param overlay: if we want to overlay a curve over the matrix
    :param force_range: forces a range between elements are plotted
    :return:
    """
    def full_rename(namelist, subtypes):
        ...
        for idux, subtype in enumerate(subtypes, 1):
            new_vert_names = rename(
                new_vert_names, subtype, ''.join(
                    ['*'] * idux))
    ...
    if colormap == 'b_jet':
        ...
        costum_cmap = colors.LinearSegmentedColormap.from_list(
            'my_colormap', prism_vals)
    ...
    if isinstance(vert_names, list):
        plt.yticks(
            ...
            vert_names,
            rotation='horizontal')
    if isinstance(horiz_names, list):
        plt.xticks(
            range(
                0,
                len(horiz_names)),
            horiz_names,
            rotation='vertical')
    if isinstance(vert_names, tuple):
        ...
        plt.yticks(
            ...
            vert_names,
            rotation='horizontal')
    if isinstance(horiz_names, tuple):
...
def list_get_indexes(lst, index_list):
    """
